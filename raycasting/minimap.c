...
void draw_minimap(t_game *game)
{
	...
	while (y < end_y)
	{
		...
		while (x < end_x)
		{
			...
			{
				if (game->map[y][x] == '1')
					draw_mini_square(game, (x - start_x), (y - start_y), 0x00000000);
				else if (game->map[y][x] == '0')
					draw_mini_square(game, (x - start_x), (y - start_y), 0x00FFFFFF);
				else if (game->map[y][x] == '2')
					draw_mini_square(game, (x - start_x), (y - start_y), 0x000000FF);
			}
			x++;
		}
		y++;
	}
	draw_player_mini(&game->mlx.minimap_image, (player_map_x - start_x)*MINI_CUBE_SIZE + MINI_CUBE_SIZE/2, (player_map_y - start_y)*MINI_CUBE_SIZE + MINI_CUBE_SIZE/2, 0x00FF0000, game);
